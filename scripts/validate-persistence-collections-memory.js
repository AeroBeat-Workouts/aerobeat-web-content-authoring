...
const predecessor=historicalV5Record("song-v5");await history.put(predecessor);assert.equal((await history.list()).some((entry)=>entry.key==="song-v5"),true);assert.equal((await history.getForExport("song-v5"))?.package.schemaVersion,5);await assert.rejects(()=>history.get("song-v5"),hasCode("flow_colliders_reimport_required"),"v5 bytes remain manageable but cannot be silently promoted to Flow Colliders");history.destroy();
