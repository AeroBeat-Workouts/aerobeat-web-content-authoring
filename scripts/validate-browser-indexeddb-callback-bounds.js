...
try {
  ...
  const result = await page.evaluate(async () => {
    ...
    const sourceBundle = () => { const bytes = new TextEncoder().encode(JSON.stringify({ version: "3.3.0", colorNotes: [], bombNotes: [], obstacles: [], sliders: [], burstSliders: [] })), entries = new Map([["info.dat", new TextEncoder().encode("{}")], ["hard.dat", bytes], ["expert.dat", bytes], ["song.ogg", new Uint8Array([1, 2, 3, 4])]]); return Object.freeze({ manifest: Object.freeze({ schemaId: "aerobeat.beatsaver-source-manifest.v2",infoFormatMajor:2,infoFormat:"v2",infoVersion:"2.1.0", infoPath: "Info.dat", songName: "Race Song", songAuthorName: "AeroBeat", levelAuthorName: "AeroBeat", audioPath: "song.ogg", bpm: 120, difficulties: Object.freeze([{ characteristic: "Standard", difficulty: "Expert", path: "Expert.dat",beatMapFormat:"v3",beatMapVersion:"3.3.0",notePalette:null }, { characteristic: "Standard", difficulty: "Hard", path: "Hard.dat",beatMapFormat:"v3",beatMapVersion:"3.3.0",notePalette:null }]), entries: Object.freeze([]) }), listEntryPaths() { return Object.freeze(["Info.dat", "Hard.dat", "Expert.dat", "song.ogg"]); }, readEntry(path) { const value = entries.get(path.toLowerCase()); if (!value) throw new Error("missing entry"); return Uint8Array.from(value); } }); };
