...
try {
  const run = async (url, expectedSecure) => {
    ...
    const db7CurrentV3Collection=expectedSecure?await page.evaluate(async()=>{
      ...
      const hostileRow={...packageRow,key:"hostile-generation",package:{schemaId:"hostile.song-package",schemaVersion:99,packageVersion:"99.0.0",packageId:"hostile",songName:"Hostile"},packageHash:`sha256:${"f".repeat(64)}`},malformedV4Row={...packageRow,key:"malformed-v4",package:{...packageValue,schemaId:"aerobeat.song-package.v4",schemaVersion:4,packageVersion:"4.0.0",notePalette:{hostile:true},charts:[{...packageValue.charts[0],schemaId:"aerobeat.chart.flow.v4",schemaVersion:4,notePalette:null}]},packageHash:`sha256:${"e".repeat(64)}`};await new Promise((resolve,reject)=>{const request=indexedDB.open(name);request.onerror=()=>reject(request.error);request.onsuccess=()=>{const database=request.result,transaction=database.transaction("packages","readwrite"),store=transaction.objectStore("packages");store.put(hostileRow);store.put(malformedV4Row);transaction.oncomplete=()=>{database.close();resolve(undefined);};transaction.onerror=()=>reject(transaction.error);};});
      const hostile=await adapter.get("hostile-generation").then(()=>"fulfilled",error=>error.code),hostileExport=await adapter.getForExport("hostile-generation"),malformedV4=await adapter.get("malformed-v4").then(()=>"fulfilled",error=>error.code),malformedV4Export=await adapter.getForExport("malformed-v4");await adapter.delete("hostile-generation");await adapter.delete("malformed-v4");
