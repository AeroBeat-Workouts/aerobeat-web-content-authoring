...
let difficultyGetterCalls=0;const maliciousDifficulty={};Object.defineProperty(maliciousDifficulty,"characteristic",{enumerable:true,get(){difficultyGetterCalls+=1;return"Standard";}});const secureBatchSource={...source,manifest:{...source.manifest,difficulties:[maliciousDifficulty,...source.manifest.difficulties]}};await assert.rejects(()=>prepareAllStandardSourceMaterials(secureBatchSource,{}),hasCode("source_manifest_invalid"));assert.equal(difficultyGetterCalls,0,"batch manifest accessors must not execute");
let legacyWorkerCalls=0;const legacyWorker={kind:"inline",async convert(){legacyWorkerCalls+=1;throw new Error("legacy source reached Worker");},destroy(){}};const currentV2Source=sourceBundle(encoder.encode("{}"),new Uint8Array([1])),legacyV1Source={...currentV2Source,manifest:{schemaId:"aerobeat.beatsaver-source-manifest.v1",sourceFormatMajor:3,infoPath:"Info.dat",songName:"Legacy",songAuthorName:"",levelAuthorName:"",audioPath:"song.ogg",bpm:120,difficulties:[{characteristic:"Standard",difficulty:"Hard",path:"Hard.dat"}],entries:[]}};let legacyReads=0;const guardedLegacySource={...legacyV1Source,readEntry(path){legacyReads+=1;return legacyV1Source.readEntry(path);}};const legacyService=createAeroWebContentAuthoringService({worker:legacyWorker});await assert.rejects(()=>legacyService.convertAndPersist(guardedLegacySource,{difficulty:"Hard"}),hasCode("source_manifest_invalid"));assert.equal(legacyReads,0,"v1 source rejection must occur before reads");assert.equal(legacyWorkerCalls,0,"v1 source rejection must occur before Worker dispatch");assert.equal((await legacyService.listPackages()).length,0,"v1 source rejection must not persist output");legacyService.destroy();
...
const predecessorWorker={kind:"inline",async convert(workerValue,runtime){const result=/** @type {Record<string,unknown>} */(structuredClone(await executeWorkerConversion(workerValue,runtime))),pkg=/** @type {Record<string,unknown>} */(result.package),flow=/** @type {Record<string,unknown>[]} */(pkg.charts).find((chart)=>chart.mode==="flow"),trace=/** @type {Record<string,unknown>[]} */(/** @type {Record<string,unknown>} */(pkg.conversionTrace).flow)[0];pkg.schemaId="aerobeat.song-package.v5";pkg.schemaVersion=5;pkg.packageVersion="5.0.0";flow.schemaId="aerobeat.chart.flow.v4";flow.schemaVersion=4;delete flow.rulesetVariants;delete trace.rulesetVariants;flow.contentHash=await prefixedSha256(canonicalJson({beats:flow.beats,rulesetId:flow.rulesetId,notePalette:flow.notePalette}));trace.contentHash=flow.contentHash;result.packageHash=await prefixedSha256(canonicalJson(pkg));result.semanticParityHash=await semanticParityHash(pkg);return result;},destroy(){}};const predecessorService=createAeroWebContentAuthoringService({worker:predecessorWorker});await assert.rejects(()=>predecessorService.convertAndPersist(boundSource,{difficulty:"Hard",sourceId:"audit",sourceVersionHash:"v1",includeAudio:false}),hasCode("worker_result_invalid"),"main must reject a correctly rehashed v5 Worker result rather than silently promote it");assert.deepEqual(await predecessorService.listPackages(),[]);predecessorService.destroy();
...
const v1Manifest={schemaId:"aerobeat.authoring-source.v1",sourceFormatMajor:3,infoPath:request.manifest.infoPath,songName:request.manifest.songName,songAuthorName:request.manifest.songAuthorName,levelAuthorName:request.manifest.levelAuthorName,bpm:request.manifest.bpm,audioPath:request.manifest.audioPath,audioContentHash:request.manifest.audioContentHash,selectedDifficulty:{difficulty:"Hard",path:"hard.dat",contentHash:request.manifest.selectedDifficulty.contentHash},sourceProvider:"synthetic",sourceId:"audit",sourceVersionHash:"v1"};
await assert.rejects(()=>executeWorkerConversion({...request,manifest:v1Manifest}),hasCode("worker_request_invalid"),"v2 envelope plus v1 manifest must reject");
await assert.rejects(()=>executeWorkerConversion({...request,version:1}),hasCode("worker_request_invalid"),"v1 envelope plus v2 manifest must reject");
await assert.rejects(()=>executeWorkerConversion({...request,manifest:{...request.manifest,sourceFormatMajor:3}}),hasCode("worker_request_invalid"),"stale sourceFormatMajor must reject instead of aliasing split format fields");
const nullBeatmapVersion=structuredClone(request);nullBeatmapVersion.manifest.selectedDifficulty.beatMapVersion=null;nullBeatmapVersion.options.sourceBeatmapVersion=null;await executeWorkerConversion(nullBeatmapVersion);
let formatCoercions=0;const hostileFormat={toString(){formatCoercions+=1;return"v3";}};await assert.rejects(()=>executeWorkerConversion({...request,manifest:{...request.manifest,selectedDifficulty:{...request.manifest.selectedDifficulty,beatMapFormat:hostileFormat}}}),hasCode("worker_request_invalid"));await assert.rejects(()=>executeWorkerConversion({...request,manifest:{...request.manifest,infoFormat:hostileFormat}}),hasCode("worker_request_invalid"));assert.equal(formatCoercions,0,"Worker format validation must never execute attacker-controlled coercion hooks");
...
const badMessage = adapterOne.convert(request); fakeOne.emit({ schema: "aerobeat/authoring_worker_message", version: 2, kind: "result", jobId: "wrong", result: {} });
await assert.rejects(() => badMessage, hasCode("worker_protocol_invalid"));
const honestResult=await executeWorkerConversion(request);
const fakeMixedResult=new FakeWorker(),adapterMixedResult=createBrowserAuthoringWorkerAdapter({workerFactory:()=>/** @type {Worker} */(/** @type {unknown} */(fakeMixedResult))});const mixedResultJob=adapterMixedResult.convert(request);fakeMixedResult.emit({schema:"aerobeat/authoring_worker_message",version:2,kind:"result",jobId:request.jobId,result:{...honestResult,version:1}});await assert.rejects(()=>mixedResultJob,hasCode("worker_protocol_invalid"),"v2 message plus v1 result must reject");
const fakeMixedMessage=new FakeWorker(),adapterMixedMessage=createBrowserAuthoringWorkerAdapter({workerFactory:()=>/** @type {Worker} */(/** @type {unknown} */(fakeMixedMessage))});const mixedMessageJob=adapterMixedMessage.convert(request);fakeMixedMessage.emit({schema:"aerobeat/authoring_worker_message",version:1,kind:"result",jobId:request.jobId,result:honestResult});await assert.rejects(()=>mixedMessageJob,hasCode("worker_protocol_invalid"),"v1 message plus v2 result must reject");
