...
function sourceBundle(difficulty, audioBytes, listedOverride) { const entries = new Map([["hard.dat", difficulty], ["song.ogg", audioBytes], ["info.dat", encoder.encode("{}")]]); return Object.freeze({ manifest: Object.freeze({schemaId:"aerobeat.beatsaver-source-manifest.v2",infoFormatMajor:2,infoFormat:"v2",infoVersion:"2.1.0", infoPath: "Info.dat", songName: "Audit", bpm: 120, audioPath: "song.ogg", difficulties: Object.freeze([{ characteristic: "Standard", difficulty: "Hard", path: "Hard.dat",beatMapFormat:"v3",beatMapVersion:"3.3.0",notePalette:null }]) }), listEntryPaths() { return listedOverride ?? ["Info.dat", "Hard.dat", "song.ogg"]; }, readEntry(path) { const value=entries.get(path.toLowerCase()); if(!value)throw new Error("missing"); return value; } }); }
