...
const migrated = createIndexedDbPersistenceAdapter({ indexedDB, databaseName: oldName }); await assert.rejects(() => migrated.get("old"), hasCode("flow_orientation_reimport_required"), "pre-v4 packages must be retained but blocked from loading because their Flow orientation is not trustworthy"); assert.equal((await migrated.list()).length, 1); migrated.destroy();
