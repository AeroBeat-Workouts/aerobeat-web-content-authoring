...
const incompatibleFormats=structuredClone(authored.package);incompatibleFormats.source.sourceInfoFormat="v4";for(const trace of [...incompatibleFormats.conversionTrace.boxing,...incompatibleFormats.conversionTrace.flow])trace.sourceInfoFormat="v4";const incompatibleValidation=await validateAuthoredPackage(incompatibleFormats);assert.equal(incompatibleValidation.issues.some((issue)=>issue.code==="source_format_provenance_invalid"),true,"package validation must reject incompatible Info v4 + beatmap v3 provenance");
const invalidDifficultyHash=structuredClone(authored.package);invalidDifficultyHash.source.sourceDifficultyHash="not-a-hash";const invalidDifficultyValidation=await validateAuthoredPackage(invalidDifficultyHash);assert.equal(invalidDifficultyValidation.issues.some((issue)=>issue.code==="source_format_provenance_invalid"),true,"package validation must require an exact difficulty hash");
const boxingTraceTamper=structuredClone(authored.package);boxingTraceTamper.conversionTrace.boxing[0].sourceInfoHash=`sha256:${"a".repeat(64)}`;const boxingTraceValidation=await validateAuthoredPackage(boxingTraceTamper);assert.equal(boxingTraceValidation.issues.some((issue)=>issue.code==="boxing_trace_invalid"),true,"package validation must reject Boxing trace source-provenance tampering");
