 ...
 * @param {{difficulty: Difficulty, songToken: string, songName: string, bpm: number, sourceProvider: string, sourceId: string, sourceVersionHash: string, sourceDifficultyPath: string, sourceBeatmapVersion: string, sourceDifficultyHash?: string, modifiers?: readonly string[], presentationSuggestion?: Readonly<Record<string, unknown>>}} options
...
function obstaclesFor(obstacles, bpm) { return obstacles.map((entry, index) => { const start = Number(entry.start ?? 0); const duration = Math.max(Number(entry.duration ?? 0), 0); return { startBeat: start, endBeat: start + duration, startMs: beatToMs(start, bpm) - timingWindowMs, endMs: beatToMs(start + duration, bpm) + timingWindowMs, blockedCells: cellsForObstacle(entry), sourceIndex: Number(entry.sourceIndex ?? index) }; }); }
/** @param {Readonly<Record<string, unknown>>} obstacle */
function cellsForObstacle(obstacle) { const x = clamp(Number(obstacle.x ?? 0), 0, 3); const width = clamp(Number(obstacle.width ?? 1), 1, 4 - x); const y = clamp(Number(obstacle.y ?? 0), 0, 2); const height = clamp(Number(obstacle.height ?? 3), 1, 3 - y); const cells = []; for (let sourceRow = y; sourceRow < y + height; sourceRow += 1) for (let column = x; column < x + width; column += 1) cells.push((2 - sourceRow) * 4 + column); return cells.sort((a, b) => a - b); }
/** @param {Readonly<Record<string, unknown>>} obstacle */
function sourceCellsForObstacle(obstacle) { const x = clamp(Number(obstacle.x ?? 0), 0, 3); const width = clamp(Number(obstacle.width ?? 1), 1, 4 - x); const y = clamp(Number(obstacle.y ?? 0), 0, 2); const height = clamp(Number(obstacle.height ?? 3), 1, 3 - y); const cells = []; for (let row = y; row < y + height; row += 1) for (let column = x; column < x + width; column += 1) cells.push(row * 4 + column); return cells.sort((a, b) => a - b); }
...
function convertFlowChart(summary,difficulty,songToken){const beats=[];const events=[];const lookup=buildFlowNoteLookup(summary.colorNotes??[]);for(const note of summary.colorNotes??[]){const emitted=emitFlowNote(note);beats.push(emitted);events.push({start:Number(note.start??0),sourceFamily:"note",result:{action:"emit",beat:cloneData(emitted),noteRef:flowNoteRef(note)},note:cloneData(note)});}for(const bomb of summary.bombNotes??[]){const emitted={start:Number(bomb.start??0),type:"bomb",placement:Number(bomb.cell??0)};beats.push(emitted);events.push({start:emitted.start,sourceFamily:"bomb",result:{action:"emit",beat:cloneData(emitted)},bomb:cloneData(bomb)});}for(const obstacle of summary.obstacles??[]){const emitted={start:Number(obstacle.start??0),end:Number(obstacle.start??0)+Number(obstacle.duration??0),type:"obstacle",cells:sourceCellsForObstacle(obstacle)};beats.push(emitted);events.push({start:emitted.start,sourceFamily:"obstacle",result:{action:"emit",beat:cloneData(emitted)},obstacle:cloneData(obstacle)});}for(const slider of summary.sliders??[]){const emitted=emitFlowArc(slider,lookup);beats.push(emitted);events.push({start:Number(slider.start??0),sourceFamily:"slider",result:{action:"emit",beat:cloneData(emitted)},slider:cloneData(slider)});}for(const burst of summary.burstSliders??[]){const emitted={start:Number(burst.start??0),end:Number(burst.end??burst.start??0),type:"burst",hand:String(burst.hand??"left"),placement:Number(burst.cell??0),direction:Number(burst.direction??8),tailPlacement:Number(burst.tailCell??burst.cell??0),checkpointCount:Math.max(Number(burst.sliceCount??1),1)};if(Object.hasOwn(burst,"spacingBias"))Object.assign(emitted,{spacingBias:Number(burst.spacingBias)});beats.push(emitted);events.push({start:emitted.start,sourceFamily:"burstSlider",result:{action:"emit",beat:cloneData(emitted)},source:cloneData(burst)});}const order={note:0,bomb:1,obstacle:2,arc:3,burst:4};beats.sort((a,b)=>Number(a.start)-Number(b.start)||(order[/** @type {keyof typeof order} */(a.type)]??99)-(order[/** @type {keyof typeof order} */(b.type)]??99)||JSON.stringify(a).localeCompare(JSON.stringify(b)));return{chart:{schemaId:"aerobeat.chart.flow.v1",schemaVersion:1,recordVersion:1,chartId:`ab-chart-${songToken}-flow-${difficulty.toLowerCase()}`,chartName:`${titleize(songToken)} ${difficulty} Flow`,mode:"flow",difficulty,beats},trace:{difficulty,events}};}
/** @param {Readonly<Record<string, unknown>>} note */
function emitFlowNote(note){const direction=Number(note.direction??8);const beat={start:Number(note.start??0),type:"note",hand:String(note.hand??"left"),placement:Number(note.cell??0),requiresDirection:direction!==8,angleOffset:Number(note.angleOffset??0)};if(direction!==8)Object.assign(beat,{direction});return beat;}
/** @param {Readonly<Record<string, unknown>>} slider @param {Map<string,string>} lookup */
function emitFlowArc(slider,lookup){const arc={start:Number(slider.start??0),end:Number(slider.end??slider.start??0),type:"arc",hand:String(slider.hand??"left"),startPlacement:Number(slider.cell??0),endPlacement:Number(slider.tailCell??slider.cell??0),startDirection:Number(slider.direction??8),endDirection:Number(slider.tailDirection??slider.direction??8),headCurveMultiplier:Number(slider.headCurveMultiplier??1),tailCurveMultiplier:Number(slider.tailCurveMultiplier??1),midAnchorMode:Number(slider.midAnchorMode??0)};const start=lookup.get(flowNoteKey(arc.start,arc.hand,arc.startPlacement));const end=lookup.get(flowNoteKey(arc.end,arc.hand,arc.endPlacement));if(start)Object.assign(arc,{startNoteRef:start});if(end)Object.assign(arc,{endNoteRef:end});return arc;}
