  ...
  const service = {
    ...
    async convertAndPersist(acquired, requestOptions) {
      ...
      try {
        ...
        const workerRequest = {
          schema: "aerobeat/authoring_worker_request", version: 2, kind: "convert", jobId,
          ...
          options: {
            ...
            noteJumpMovementSpeed: manifest.selectedDifficulty.noteJumpMovementSpeed, noteJumpStartBeatOffset: manifest.selectedDifficulty.noteJumpStartBeatOffset, spawnTiming: cloneData(manifest.selectedDifficulty.spawnTiming),
            sourceProvider: manifest.sourceProvider, sourceId: manifest.sourceId, sourceVersionHash: manifest.sourceVersionHash,
            sourceInfoFormat: manifest.infoFormat, sourceInfoVersion: manifest.infoVersion, sourceInfoHash: manifest.infoHash,
            sourceDifficultyPath: manifest.selectedDifficulty.path, sourceBeatmapFormat: manifest.selectedDifficulty.beatMapFormat, sourceBeatmapVersion: manifest.selectedDifficulty.beatMapVersion,
        ...
        if (!trustedValidation.valid || trustedValidation.packageHash !== resultPackageHash || !await workerResultMatchesManifestFinal(resultPackage,manifest,normalizedOptions.includeAudio!==false,converterProfile,resultParityHash,resultSourceHash)) throw authoringError("worker_result_invalid", "Worker package failed main-thread validation, source/profile/projection binding or hash verification");
        checkCurrent(generation, abort.signal);
    ...
    async convertAllStandardAndPersist(acquired, requestOptions) {
      ...
      try {
        ...
        for (let index = 0; index < prepared.materials.length; index += 1) {
          ...
          const workerRequest = {
            schema: "aerobeat/authoring_worker_request", version: 2, kind: "convert", jobId: `${jobId}-${index + 1}`,
            manifest: cloneData(manifest), difficultyBytes: Uint8Array.from(material.difficultyBytes),
            options: {
          ...
          if (!isPlainRecord(resultPackage) || typeof resultPackageHash !== "string" || typeof resultParityHash !== "string" || typeof resultSourceHash !== "string") throw authoringError("worker_result_invalid", "Worker did not return a validated package");
          const trustedValidation = await validateAuthoredPackage(resultPackage);
          if (!trustedValidation.valid || trustedValidation.packageHash !== resultPackageHash || !await workerResultMatchesManifestFinal(resultPackage, manifest, true, converterProfile, resultParityHash, resultSourceHash)) throw authoringError("worker_result_invalid", "Worker batch package failed validation or source/profile binding");
...
async function workerResultMatchesManifestFinal(packageValue,manifest,includeAudio,converterProfile,workerParityHash,workerSourceHash){
  try{
    const source=dataProperty(packageValue,"source"),selected=dataProperty(manifest,"selectedDifficulty"),song=dataProperty(packageValue,"song"),charts=dataProperty(packageValue,"charts"),trace=dataProperty(packageValue,"conversionTrace");
    if(!isPlainRecord(source)||!isPlainRecord(selected)||!isPlainRecord(song)||!Array.isArray(charts)||!isPlainRecord(trace))return false;
    const sourcePalette=verifySourceNotePalette(dataProperty(selected,"notePalette"),{infoFormat:/** @type {"v2"|"v4"} */(dataProperty(manifest,"infoFormat")),infoHash:/** @type {string} */(dataProperty(manifest,"infoHash")),difficultyHash:/** @type {string} */(dataProperty(selected,"contentHash"))});
    const expectedAuthoredPalette=await createAuthoredNotePalette(sourcePalette),expectedPaletteReference=flowPaletteReference(expectedAuthoredPalette);
    if(canonicalJson(dataProperty(packageValue,"notePalette"))!==canonicalJson(expectedAuthoredPalette)||canonicalJson(dataProperty(trace,"notePalette"))!==canonicalJson(expectedPaletteReference))return false;
    const expectedSpawnTiming=verifyBeatSaberSpawnTiming(dataProperty(selected,"spawnTiming"));
    const expected={provider:dataProperty(manifest,"sourceProvider"),sourceId:dataProperty(manifest,"sourceId"),sourceVersionHash:dataProperty(manifest,"sourceVersionHash"),difficulty:dataProperty(selected,"difficulty"),sourceInfoFormat:dataProperty(manifest,"infoFormat"),sourceInfoVersion:dataProperty(manifest,"infoVersion"),sourceInfoHash:dataProperty(manifest,"infoHash"),sourceDifficultyPath:dataProperty(selected,"path"),sourceBeatmapFormat:dataProperty(selected,"beatMapFormat"),sourceBeatmapVersion:dataProperty(selected,"beatMapVersion"),sourceDifficultyHash:dataProperty(selected,"contentHash"),sourceHash:workerSourceHash,obstacleContract:"normalized_obstacle_v2"};
    for(const [key,value] of Object.entries(expected))if(dataProperty(source,key)!==value)return false;
    if(canonicalJson(dataProperty(source,"spawnTiming"))!==canonicalJson(expectedSpawnTiming)||canonicalJson(dataProperty(trace,"spawnTiming"))!==canonicalJson(expectedSpawnTiming))return false;
    const expectedProfile=converterProfile===null?undefined:canonicalJson(converterProfile);if((converterProfile===null&&(dataProperty(source,"converterProfile")!==undefined||dataProperty(trace,"converterProfile")!==undefined))||(converterProfile!==null&&(canonicalJson(dataProperty(source,"converterProfile"))!==expectedProfile||canonicalJson(dataProperty(trace,"converterProfile"))!==expectedProfile)))return false;
    if(charts.length!==5||charts.some((chart)=>!isPlainRecord(chart)||dataProperty(chart,"difficulty")!==dataProperty(selected,"difficulty")))return false;
    const boxing=dataProperty(trace,"boxing"),flow=dataProperty(trace,"flow");if(!Array.isArray(boxing)||boxing.length!==4||!Array.isArray(flow)||flow.length!==1)return false;
    for(const item of boxing)if(!isPlainRecord(item)||dataProperty(item,"sourceHash")!==workerSourceHash||dataProperty(item,"sourceInfoFormat")!==expected.sourceInfoFormat||dataProperty(item,"sourceInfoVersion")!==expected.sourceInfoVersion||dataProperty(item,"sourceInfoHash")!==expected.sourceInfoHash||dataProperty(item,"sourceDifficultyPath")!==expected.sourceDifficultyPath||dataProperty(item,"sourceBeatmapFormat")!==expected.sourceBeatmapFormat||dataProperty(item,"sourceBeatmapVersion")!==expected.sourceBeatmapVersion||dataProperty(item,"sourceDifficultyHash")!==expected.sourceDifficultyHash||canonicalJson(dataProperty(item,"spawnTiming"))!==canonicalJson(expectedSpawnTiming))return false;
    const flowItem=flow[0],flowChart=charts.find((chart)=>isPlainRecord(chart)&&dataProperty(chart,"mode")==="flow");if(!isPlainRecord(flowItem)||!isPlainRecord(flowChart)||dataProperty(flowItem,"sourceHash")!==workerSourceHash||dataProperty(flowItem,"sourceInfoFormat")!==expected.sourceInfoFormat||dataProperty(flowItem,"sourceInfoVersion")!==expected.sourceInfoVersion||dataProperty(flowItem,"sourceInfoHash")!==expected.sourceInfoHash||dataProperty(flowItem,"sourceDifficultyPath")!==expected.sourceDifficultyPath||dataProperty(flowItem,"sourceBeatmapFormat")!==expected.sourceBeatmapFormat||dataProperty(flowItem,"sourceBeatmapVersion")!==expected.sourceBeatmapVersion||dataProperty(flowItem,"sourceDifficultyHash")!==expected.sourceDifficultyHash||canonicalJson(dataProperty(flowItem,"spawnTiming"))!==canonicalJson(expectedSpawnTiming)||canonicalJson(dataProperty(flowItem,"notePalette"))!==canonicalJson(expectedPaletteReference)||canonicalJson(dataProperty(flowChart,"notePalette"))!==canonicalJson(expectedPaletteReference)||dataProperty(flowItem,"contentHash")!==dataProperty(flowChart,"contentHash"))return false;
    const audio=dataProperty(song,"audio");if(includeAudio?(!isPlainRecord(audio)||dataProperty(audio,"filePath")!==dataProperty(manifest,"audioPath")||dataProperty(audio,"contentHash")!==dataProperty(manifest,"audioContentHash")):audio!==undefined)return false;
    return await semanticParityHash(packageValue)===workerParityHash;
  }catch{return false;}
}
