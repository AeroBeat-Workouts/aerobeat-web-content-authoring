...
export function createMemoryPersistenceAdapter(options = {}) {
  ...
  return Object.freeze({
    ...
    async put(record) { assertOpen(); const copy = copyRecord(record, correctedFlowCellOrientation, true); const staged = new Map(records); staged.set(copy.key, copy); assertQuota(staged, sharedAssets, collections); records.set(copy.key, copy); },
    /** @param {string} key */
...
export function createIndexedDbPersistenceAdapter(options = {}) {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (request.transaction) {
          ...
          if (event.oldVersion > 0 && event.oldVersion < authoringDatabaseVersion) {
            ...
            packageCursor.onsuccess = () => { const cursor=packageCursor.result;if(!cursor)return;const value=cursor.value;cursor.update({...value,sourceCache:Array.isArray(value.sourceCache)?value.sourceCache:[],writeToken:typeof value.writeToken==="string"?value.writeToken:"",schemaVersion:authoringDatabaseVersion,flowCellOrientation:legacyFlowCellOrientation});cursor.continue(); };
            const collectionCursor = transaction.objectStore("collections").openCursor();
            collectionCursor.onsuccess = () => { const cursor=collectionCursor.result;if(!cursor)return;cursor.update({...cursor.value,schemaVersion:authoringDatabaseVersion,flowCellOrientation:legacyFlowCellOrientation});cursor.continue(); };
            transaction.objectStore("meta").put({ key: "flow-orientation-invalidation", invalidatedBeforeVersion: authoringDatabaseVersion, mode: "preserved-reimport-required" });
          }
        }
      };
  ...
  return Object.freeze({
    ...
    schemaVersion: authoringDatabaseVersion,
    /** @param {StoredPackageRecord} record */
    async put(record) { await putIndexedDbPackage(await open(),copyRecord(record, correctedFlowCellOrientation, true)); },
    /** @param {string} key */
...
function copyRecord(record,assumedOrientation=legacyFlowCellOrientation,forceOrientation=false) { const baseKeys=["key","package","packageHash","assets","sourceCache","createdAtMs","schemaVersion","writeToken"],refKeys=[...baseKeys,"assetRefs"],orientedBaseKeys=[...baseKeys,"flowCellOrientation"],orientedRefKeys=[...refKeys,"flowCellOrientation"];if(!exactRecord(record,baseKeys)&&!exactRecord(record,refKeys)&&!exactRecord(record,orientedBaseKeys)&&!exactRecord(record,orientedRefKeys))throw storageError("storage_record_invalid","Stored package record shape is invalid");const key=valueFor(record,"key"),packageValue=valueFor(record,"package"),packageHash=valueFor(record,"packageHash"),createdAtMs=valueFor(record,"createdAtMs"),writeToken=valueFor(record,"writeToken"),storedOrientation=valueFor(record,"flowCellOrientation"),flowCellOrientation=forceOrientation||storedOrientation===undefined?assumedOrientation:storedOrientation;if(typeof key!=="string"||!key||key.length>1024||!isPlainRecord(packageValue)||typeof packageHash!=="string"||!/^sha256:[0-9a-f]{64}$/u.test(packageHash)||!Number.isSafeInteger(createdAtMs)||Number(createdAtMs)<0||typeof writeToken!=="string"||writeToken.length>128||(flowCellOrientation!==correctedFlowCellOrientation&&flowCellOrientation!==legacyFlowCellOrientation))throw storageError("storage_record_invalid","Stored package record values are invalid");let encoded;try{encoded=canonicalJson(packageValue);}catch{throw storageError("storage_record_invalid","Stored package must contain canonical plain data");}if(new TextEncoder().encode(encoded).byteLength>64*1024*1024)throw storageError("storage_record_invalid","Stored package exceeds the size limit");const result={ key, package: /** @type {Record<string, unknown>} */ (cloneData(packageValue)), packageHash, assets: copyAssets(valueFor(record,"assets")), sourceCache: copyAssets(valueFor(record,"sourceCache")), createdAtMs, schemaVersion: authoringDatabaseVersion, writeToken, flowCellOrientation };const refs=valueFor(record,"assetRefs");return refs===undefined?result:{...result,assetRefs:copyAssetRefs(refs)}; }
/** @param {StoredPackageRecord} record */
...
/** @param {StoredPackageRecord} record */
function legacyCollection(record){const summary=summaryFor(record),id=`legacy:${record.key}`;return {collectionId:id,songName:summary.songName||"Downloaded song",sourceProvider:"legacy",sourceId:record.key,sourceVersionHash:"legacy",converterProfileId:"legacy",converterProfileHash:"legacy",modifierIds:[],packageKeys:[record.key],packages:[{packageKey:record.key,packageId:summary.packageId||record.key,difficultyId:summary.difficulty||"default",difficultyLabel:summary.difficulty||"Default"}],createdAtMs:record.createdAtMs,schemaVersion:authoringDatabaseVersion,writeToken:record.writeToken,flowCellOrientation:record.flowCellOrientation??legacyFlowCellOrientation};}
