...
export function createIndexedDbPersistenceAdapter(options = {}) {
  ...
  return Object.freeze({
    ...
    /** @param {string} key */
    async get(key) { return getIndexedDbPackage(await open(), key); },
...
function getIndexedDbPackage(database,key){return new Promise((resolve,reject)=>{const tx=database.transaction(["packages","assets"],"readonly"),request=tx.objectStore("packages").get(key);let result=null;request.onsuccess=()=>{if(!request.result)return;const record=copyRecord(request.result);if(!record.assetRefs?.length){result=record;return;}const resolved=new Map();for(const ref of record.assetRefs){const assetRequest=tx.objectStore("assets").get(ref.contentHash);assetRequest.onsuccess=()=>{if(assetRequest.result)resolved.set(ref.contentHash,copySharedAssetRecord(assetRequest.result));};}tx.oncomplete=()=>{try{result=resolveRecordAssets(record,resolved);resolve(result);}catch(error){reject(error);}};};request.onerror=()=>reject(idbStorageError(request.error,"indexeddb_request_failed","IndexedDB package lookup failed"));tx.oncomplete=()=>resolve(result);tx.onerror=()=>reject(idbStorageError(tx.error,"indexeddb_transaction_failed","IndexedDB package transaction failed"));tx.onabort=()=>reject(idbStorageError(tx.error,"indexeddb_transaction_aborted","IndexedDB package transaction aborted"));});}
