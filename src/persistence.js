...
/** @param {IDBTransaction} tx @param {()=>void} callback @param {((error:Error)=>void) | undefined} [onCompletedError] */
function guardedIdbCallback(tx,callback,onCompletedError){return ()=>{if(transactionCallbackErrors.has(tx))return;try{callback();}catch(error){const bounded=boundedStorageError(error,"storage_record_invalid","Stored authoring data is invalid");transactionCallbackErrors.set(tx,bounded);try{tx.abort();}catch{}onCompletedError?.(bounded);}};}
