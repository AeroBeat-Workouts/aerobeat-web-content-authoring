      ...
      request.onupgradeneeded = (event) => {
        ...
        if (request.transaction) {
          ...
          if (event.oldVersion > 0 && event.oldVersion < authoringDatabaseVersion) {
            const failUpgrade = (error) => { if(upgradeError)return;upgradeError=boundedStorageError(error,"storage_migration_failed","Stored authoring data could not be migrated");try{transaction.abort();}catch{} };
            const packageCursor = transaction.objectStore("packages").openCursor();
            packageCursor.onsuccess = () => { if(upgradeError)return;try { const cursor=packageCursor.result;if(!cursor)return;cursor.update(migratePackageRecord(cursor.value,event.oldVersion));cursor.continue(); } catch(error) { failUpgrade(error); } };
            const collectionCursor = transaction.objectStore("collections").openCursor();
            collectionCursor.onsuccess = () => { if(upgradeError)return;try { const cursor=collectionCursor.result;if(!cursor)return;cursor.update(migrateCollectionRecord(cursor.value,event.oldVersion));cursor.continue(); } catch(error) { failUpgrade(error); } };
...
function putIndexedDbPackage(database,record){return new Promise((resolve,reject)=>{const tx=database.transaction(["packages","assets","collections"],"readwrite");tx.objectStore("packages").put(record);garbageCollectIndexedDbAssets(tx);tx.oncomplete=()=>resolve(undefined);tx.onerror=()=>reject(transactionStorageError(tx,"indexeddb_transaction_failed","IndexedDB package transaction failed"));tx.onabort=()=>reject(transactionStorageError(tx,"indexeddb_transaction_aborted","IndexedDB package transaction aborted"));});}
/** @param {IDBDatabase} database @param {string} key @param {boolean} allowStale */
function getIndexedDbPackage(database,key,allowStale){return new Promise((resolve,reject)=>{const tx=database.transaction(["packages","assets"],"readonly"),request=tx.objectStore("packages").get(key);let result=null;request.onsuccess=guardedIdbCallback(tx,()=>{if(!request.result)return;const record=copyRecord(request.result);if(!record.assetRefs?.length){result=resolveRecordAssets(record,new Map(),allowStale);return;}const resolved=new Map();for(const ref of record.assetRefs){const assetRequest=tx.objectStore("assets").get(ref.contentHash);assetRequest.onsuccess=guardedIdbCallback(tx,()=>{if(assetRequest.result)resolved.set(ref.contentHash,copySharedAssetRecord(assetRequest.result));});}tx.oncomplete=guardedIdbCallback(tx,()=>{result=resolveRecordAssets(record,resolved,allowStale);resolve(result);},reject);});request.onerror=()=>reject(idbStorageError(request.error,"indexeddb_request_failed","IndexedDB package lookup failed"));tx.oncomplete=()=>resolve(result);tx.onerror=()=>reject(transactionStorageError(tx,"indexeddb_transaction_failed","IndexedDB package transaction failed"));tx.onabort=()=>reject(transactionStorageError(tx,"indexeddb_transaction_aborted","IndexedDB package transaction aborted"));});}
...
function putIndexedDbCollection(database,batch,signal){if(signal?.aborted)return Promise.reject(storageError("operation_aborted","Persistence operation was cancelled"));return new Promise((resolve,reject)=>{const tx=database.transaction(["assets","packages","collections"],"readwrite");let abortRequested=false,terminal=false;const cleanup=()=>signal?.removeEventListener("abort",cancel),safeAbort=()=>{if(abortRequested||terminal)return;abortRequested=true;try{tx.abort();}catch(error){if(!(error instanceof DOMException)||error.name!=="InvalidStateError")throw error;}},cancel=()=>{if(terminal||transactionCallbackErrors.has(tx))return;transactionCallbackErrors.set(tx,storageError("operation_aborted","Persistence operation was cancelled"));safeAbort();},terminalError=(code,message)=>transactionStorageError(tx,code,message);signal?.addEventListener("abort",cancel,{once:true});for(const asset of batch.assets)tx.objectStore("assets").put(asset);for(const record of batch.packages)tx.objectStore("packages").put(record);tx.objectStore("collections").put(batch.collection);garbageCollectIndexedDbAssets(tx,safeAbort);tx.oncomplete=()=>{terminal=true;cleanup();resolve(undefined);};tx.onerror=()=>{terminal=true;cleanup();reject(terminalError("indexeddb_transaction_failed","IndexedDB collection transaction failed"));};tx.onabort=()=>{terminal=true;cleanup();reject(terminalError("indexeddb_transaction_aborted","IndexedDB collection transaction aborted"));};});}
...
/** @param {IDBDatabase} database @param {string} collectionId */
function deleteIndexedDbCollection(database,collectionId){return new Promise((resolve,reject)=>{const tx=database.transaction(["packages","assets","collections"],"readwrite"),packages=tx.objectStore("packages"),collections=tx.objectStore("collections"),request=collections.get(collectionId);let deleted=false;request.onsuccess=guardedIdbCallback(tx,()=>{if(request.result){const collection=copyCollection(request.result);for(const key of collection.packageKeys)packages.delete(key);collections.delete(collectionId);deleted=true;garbageCollectIndexedDbAssets(tx);return;}if(collectionId.startsWith("legacy:")){const key=collectionId.slice(7),legacyRequest=packages.getKey(key);legacyRequest.onsuccess=guardedIdbCallback(tx,()=>{if(legacyRequest.result!==undefined){packages.delete(key);deleted=true;garbageCollectIndexedDbAssets(tx);}});}});tx.oncomplete=()=>resolve(deleted);tx.onerror=()=>reject(transactionStorageError(tx,"indexeddb_transaction_failed","IndexedDB collection delete failed"));tx.onabort=()=>reject(transactionStorageError(tx,"indexeddb_transaction_aborted","IndexedDB collection delete aborted"));});}
/** @param {IDBTransaction} tx @param {(()=>void) | undefined} [abort] */
function garbageCollectIndexedDbAssets(tx,abort){const packageRequest=tx.objectStore("packages").getAll();packageRequest.onsuccess=guardedIdbCallback(tx,()=>{const used=new Set();for(const value of packageRequest.result){const record=copyRecord(value);for(const ref of record.assetRefs??[])used.add(ref.contentHash);}const cursorRequest=tx.objectStore("assets").openCursor();cursorRequest.onsuccess=guardedIdbCallback(tx,()=>{const cursor=cursorRequest.result;if(!cursor)return;if(typeof cursor.key==="string"&&!used.has(cursor.key))cursor.delete();cursor.continue();},undefined,abort);},undefined,abort);}
/** @param {IDBTransaction} tx @param {string} key */
function removeIndexedDbPackageFromCollections(tx,key){const store=tx.objectStore("collections"),request=store.getAll();request.onsuccess=guardedIdbCallback(tx,()=>{for(const value of request.result){const collection=copyCollection(value);if(!collection.packageKeys.includes(key))continue;if(collection.packageKeys.length===1){store.delete(collection.collectionId);continue;}store.put({...collection,packageKeys:collection.packageKeys.filter((item)=>item!==key),packages:collection.packages.filter((entry)=>entry.packageKey!==key)});}});}
/** @param {unknown} value */
...
function deleteExisting(database,key){return new Promise((resolve,reject)=>{const tx=database.transaction(["packages","assets","collections"],"readwrite"),store=tx.objectStore("packages"),request=store.getKey(key);let deleted=false;request.onsuccess=guardedIdbCallback(tx,()=>{if(request.result!==undefined){store.delete(key);deleted=true;removeIndexedDbPackageFromCollections(tx,key);garbageCollectIndexedDbAssets(tx);}});request.onerror=()=>reject(idbStorageError(request.error,"indexeddb_request_failed","IndexedDB delete lookup failed"));tx.oncomplete=()=>resolve(deleted);tx.onerror=()=>reject(transactionStorageError(tx,"indexeddb_transaction_failed","IndexedDB delete transaction failed"));tx.onabort=()=>reject(transactionStorageError(tx,"indexeddb_transaction_aborted","IndexedDB delete transaction aborted"));});}
...
function conditionalDelete(database, key, writeToken) {
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = guardedIdbCallback(tx,() => { const value = request.result; if (value && value.writeToken === writeToken) { store.delete(key); removeIndexedDbPackageFromCollections(tx,key); garbageCollectIndexedDbAssets(tx); deleted = true; } });
    request.onerror = () => reject(idbStorageError(request.error,"indexeddb_request_failed","IndexedDB request failed"));
    tx.oncomplete = () => resolve(deleted); tx.onerror = () => reject(transactionStorageError(tx,"indexeddb_transaction_failed","IndexedDB transaction failed")); tx.onabort = () => reject(transactionStorageError(tx,"indexeddb_transaction_aborted","IndexedDB transaction aborted"));
  });
}
...
function obstacleContractForPackage(packageValue){
  ...
  const flow=flowCharts[0];const expectedFlowSchema=currentV4?"aerobeat.chart.flow.v4":"aerobeat.chart.flow.v3",expectedFlowVersion=currentV4?4:3;if(valueFor(flow,"schemaId")!==expectedFlowSchema||valueFor(flow,"schemaVersion")!==expectedFlowVersion||valueFor(flow,"rulesetId")!=="flow_grid_v2")return legacyObstacleContract;
...
/** Package v4 is the first palette-explicit generation. @param {Record<string,unknown>} packageValue */
function hasCurrentNotePaletteContract(packageValue){
  if(valueFor(packageValue,"schemaId")!=="aerobeat.song-package.v4"||valueFor(packageValue,"schemaVersion")!==4||valueFor(packageValue,"packageVersion")!=="4.0.0")return false;
  const palette=valueFor(packageValue,"notePalette");if(palette!==null&&!isAuthoredNotePalette(palette))return false;
  const charts=denseDataArray(valueFor(packageValue,"charts"),64);if(!charts)return false;const flow=charts.filter((chart)=>isPlainRecord(chart)&&valueFor(chart,"mode")==="flow");if(flow.length!==1||valueFor(flow[0],"schemaId")!=="aerobeat.chart.flow.v4"||valueFor(flow[0],"schemaVersion")!==4)return false;
  const reference=valueFor(flow[0],"notePalette");return palette===null?reference===null:isPlainRecord(reference)&&valueFor(reference,"source")==="package"&&valueFor(reference,"paletteHash")===palette.paletteHash;
}
...
/** @param {StoredPackageRecord} record */
function legacyCollection(record){const summary=summaryFor(record),id=`legacy:${record.key}`;return {collectionId:id,songName:summary.songName||"Downloaded song",sourceProvider:"legacy",sourceId:record.key,sourceVersionHash:"legacy",converterProfileId:"legacy",converterProfileHash:"legacy",modifierIds:[],packageKeys:[record.key],packages:[{packageKey:record.key,packageId:summary.packageId||record.key,difficultyId:summary.difficulty||"default",difficultyLabel:summary.difficulty||"Default"}],createdAtMs:record.createdAtMs,schemaVersion:authoringDatabaseVersion,writeToken:record.writeToken,flowCellOrientation:record.flowCellOrientation??legacyFlowCellOrientation,obstacleContract:record.obstacleContract??legacyObstacleContract};}
...
/** @param {IDBTransaction} tx @param {()=>void} callback @param {((error:Error)=>void) | undefined} [onCompletedError] @param {(()=>void) | undefined} [abort] */
function guardedIdbCallback(tx,callback,onCompletedError,abort){return ()=>{if(transactionCallbackErrors.has(tx))return;try{callback();}catch(error){const bounded=boundedStorageError(error,"storage_record_invalid","Stored authoring data is invalid");transactionCallbackErrors.set(tx,bounded);try{if(abort)abort();else tx.abort();}catch{}onCompletedError?.(bounded);}};}
