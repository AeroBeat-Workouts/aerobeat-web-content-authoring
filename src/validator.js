...
import { flowPaletteReference, verifyAuthoredNotePalette } from "./note-palette.js";
import { verifyBeatSaberSpawnTiming } from "./spawn-timing.js";
import { authoredPackageSchemaId, authoredPackageSchemaVersion, authoredPackageVersion, flowChartSchemaId, flowChartSchemaVersion, flowCollidersRulesetId, flowContentIdentity, flowRulesetVariants, hasExactFlowRulesetVariants } from "./flow-contract.js";
...
export async function validateAuthoredPackage(packageValue) {
  ...
  if ([1,2,3,4].some((version)=>packageValue.schemaId===`aerobeat.song-package.v${version}`&&packageValue.schemaVersion===version)) issue("spawn_timing_reimport_required", "schemaId", "Prior package versions require reimport for source spawn timing");
  ...
  else if (packageValue.schemaId !== authoredPackageSchemaId || packageValue.schemaVersion !== authoredPackageSchemaVersion || packageValue.packageVersion !== authoredPackageVersion) issue("package_schema_invalid", "schemaId", `Package schema must be ${authoredPackageSchemaId} version ${authoredPackageSchemaVersion} / package ${authoredPackageVersion}`);
  ...
  if(isPlainRecord(packageValue.source)){
    try{spawnTiming=verifyBeatSaberSpawnTiming(packageValue.source.spawnTiming);}catch(cause){issue(errorCode(cause),"source.spawnTiming",cause instanceof Error?cause.message:"Spawn timing is invalid");}
    ...
    const beatmapFormatValid=["v2","v3","v4"].includes(String(packageValue.source.sourceBeatmapFormat));
  ...
  const conversionTrace=isPlainRecord(packageValue.conversionTrace)?packageValue.conversionTrace:null;const boxingTraces=conversionTrace&&Array.isArray(conversionTrace.boxing)?conversionTrace.boxing:[];const flowTraces=conversionTrace&&Array.isArray(conversionTrace.flow)?conversionTrace.flow:[];if(boxingTraces.length!==4)issue("boxing_trace_count_invalid","conversionTrace.boxing","Conversion trace must contain exactly four Boxing traces");if(flowTraces.length!==1)issue("flow_trace_count_invalid","conversionTrace.flow","Conversion trace must contain exactly one Flow trace");if(!spawnTiming||!conversionTrace||!canonicalEqual(conversionTrace.spawnTiming,spawnTiming))issue("spawn_timing_trace_mismatch","conversionTrace.spawnTiming","Conversion trace must bind exact source spawn timing");const songTiming=isPlainRecord(packageValue.song)&&isPlainRecord(packageValue.song.timing)&&Array.isArray(packageValue.song.timing.tempoSegments)&&isPlainRecord(packageValue.song.timing.tempoSegments[0])?packageValue.song.timing.tempoSegments[0]:null;if(spawnTiming&&(!songTiming||songTiming.bpm!==spawnTiming.bpm))issue("spawn_timing_bpm_mismatch","song.timing","Spawn timing BPM must match authored song timing");const expectedPaletteReference=flowPaletteReference(isAuthoredNotePalette(notePalette)?notePalette:null);
  ...
  for(let index=0;index<boxingTraces.length;index+=1){
    ...
    const traceConverterProfile=isPlainRecord(trace)?trace.converterProfile:undefined;
    ...
    if(converterProfile){try{if(canonicalJson(traceConverterProfile)!==canonicalJson(converterProfile))issue("converter_profile_boxing_trace_mismatch",`conversionTrace.boxing[${index}].converterProfile`,"Every Boxing trace converter profile must exactly match package source provenance");}catch{issue("converter_profile_boxing_trace_mismatch",`conversionTrace.boxing[${index}].converterProfile`,"Every Boxing trace converter profile must exactly match package source provenance");}}else if(traceConverterProfile!==undefined)issue("converter_profile_unbound",`conversionTrace.boxing[${index}].converterProfile`,"Boxing trace converter profile requires package source provenance");
  }
  for(let index=0;index<flowTraces.length;index+=1){const trace=flowTraces[index];if(isPlainRecord(trace)&&trace.converterProfile!==undefined)issue("converter_profile_flow_trace_forbidden",`conversionTrace.flow[${index}].converterProfile`,"Flow traces must not carry Boxing converter profile provenance");const legacyFlowTraceAccepted=isPlainRecord(trace)&&trace.rulesetId==="flow_grid_v2"&&Array.isArray(trace.rulesetVariants)&&trace.rulesetVariants.length===2&&trace.rulesetVariants[0]==="flow_grid_v2"&&trace.rulesetVariants[1]==="flow_colliders_v1";if(!isPlainRecord(trace)||trace.obstacleContract!=="normalized_obstacle_v2"||(!legacyFlowTraceAccepted&&trace.rulesetId!==flowCollidersRulesetId)||!hasExactFlowRulesetVariants(trace.rulesetVariants)||!isPlainRecord(packageValue.source)||trace.sourceHash!==packageValue.source.sourceHash||trace.sourceInfoFormat!==packageValue.source.sourceInfoFormat||trace.sourceInfoVersion!==packageValue.source.sourceInfoVersion||trace.sourceInfoHash!==packageValue.source.sourceInfoHash||trace.sourceDifficultyPath!==packageValue.source.sourceDifficultyPath||trace.sourceBeatmapFormat!==packageValue.source.sourceBeatmapFormat||trace.sourceBeatmapVersion!==packageValue.source.sourceBeatmapVersion||trace.sourceDifficultyHash!==packageValue.source.sourceDifficultyHash||!canonicalEqual(trace.spawnTiming,packageValue.source.spawnTiming)||!canonicalEqual(trace.notePalette,expectedPaletteReference))issue("flow_trace_invalid",`conversionTrace.flow[${index}]`,"Flow trace must bind the sole Flow (colliders) ruleset variant, normalized obstacles, source provenance and palette reference");}
  ...
  for (let index = 0; index < charts.length; index += 1) {
    ...
    if (chart.mode === "flow") { flowCount += 1; const legacyFlowGridAccepted=chart.rulesetId==="flow_grid_v2"&&Array.isArray(chart.rulesetVariants)&&chart.rulesetVariants.length===2&&chart.rulesetVariants[0]==="flow_grid_v2"&&chart.rulesetVariants[1]==="flow_colliders_v1";if(chart.schemaId!==flowChartSchemaId||chart.schemaVersion!==flowChartSchemaVersion||(chart.rulesetId!==flowCollidersRulesetId&&!legacyFlowGridAccepted)||!hasExactFlowRulesetVariants(chart.rulesetVariants))issue("flow_chart_schema_invalid",path,"Flow chart must bind the sole Flow (colliders) ruleset variant; historical two-variant Flow Grid binds require reimport");if(!canonicalEqual(chart.notePalette,expectedPaletteReference))issue("flow_note_palette_mismatch",`${path}.notePalette`,"Flow chart must reference the package note palette exactly");const flowContentBindingMatches=chart.rulesetId===flowCollidersRulesetId&&canonicalEqual(chart.rulesetVariants,flowRulesetVariants);const expectedFlowContentHash=await prefixedSha256(canonicalJson(flowContentIdentity(chart.beats,expectedPaletteReference)));if(flowContentBindingMatches&&chart.contentHash!==expectedFlowContentHash)issue("flow_content_hash_mismatch",`${path}.contentHash`,"Flow content hash must bind beats, exact ruleset variants and palette reference");const flowTrace=flowTraces[0];if(isPlainRecord(flowTrace)&&traceBindingMatchesCurrent(flowTrace)&&flowTrace.contentHash!==expectedFlowContentHash)issue("flow_trace_content_hash_mismatch","conversionTrace.flow[0].contentHash","Flow trace content hash must bind the current Flow identity exactly"); const obstacleCount=chart.beats.filter((beat)=>isPlainRecord(beat)&&beat.type==="obstacle").length;if(obstacleCount>maximumObstaclesPerChart)issue("flow_obstacle_limit_exceeded",`${path}.beats`,`Flow chart exceeds the obstacle limit`);for (let beatIndex = 0; beatIndex < chart.beats.length; beatIndex += 1) validateFlowBeat(chart.beats[beatIndex], `${path}.beats[${beatIndex}]`, issue); continue; }
...
function canonicalEqual(left,right){try{return canonicalJson(left)===canonicalJson(right);}catch{return false;}}
/** @param {Readonly<Record<string, unknown>> | undefined} trace */
function traceBindingMatchesCurrent(trace){return isPlainRecord(trace)&&trace.rulesetId===flowCollidersRulesetId&&canonicalEqual(trace.rulesetVariants,flowRulesetVariants);}
/** @param {unknown} value */
function errorCode(value){if(value&&typeof value==="object"){const descriptor=Object.getOwnPropertyDescriptor(value,"code");if(descriptor&&"value" in descriptor&&typeof descriptor.value==="string")return descriptor.value;}return"spawn_timing_invalid";}
/** @param {unknown} value */
