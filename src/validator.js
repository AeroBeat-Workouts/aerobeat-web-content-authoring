...
export async function validateAuthoredPackage(packageValue) {
  ...
  for (let index = 0; index < charts.length; index += 1) {
    ...
    if (chart.mode === "flow") { flowCount += 1; if(chart.schemaId!=="aerobeat.chart.flow.v2"||chart.schemaVersion!==2||chart.rulesetId!=="flow_grid_v2")issue("flow_chart_schema_invalid",path,"Flow chart must use source-geometry schema/ruleset v2"); const obstacleCount=chart.beats.filter((beat)=>isPlainRecord(beat)&&beat.type==="obstacle").length;if(obstacleCount>maximumFlowObstaclesPerChart)issue("flow_obstacle_limit_exceeded",`${path}.beats`,`Flow chart exceeds the obstacle limit`);for (let beatIndex = 0; beatIndex < chart.beats.length; beatIndex += 1) validateFlowBeat(chart.beats[beatIndex], `${path}.beats[${beatIndex}]`, issue); continue; }
...
function validateFlowBeat(beat,path,issue){
  ...
  if(String(beat.type)==="obstacle"){
    ...
    if(Reflect.ownKeys(beat).length!==keys.length||!keys.every((key)=>Object.hasOwn(beat,key))||!Number.isFinite(beat.end)||Number(beat.end)<=Number(beat.start)||Number(beat.end)>144000||!isFlowObstacleGeometry(beat.geometry)||!isFlowObstacleGridMask(beat.gridMask,/** @type {import("@aerobeat/web-contracts/flow-obstacle-contracts").AeroFlowObstacleGeometry} */(beat.geometry)))issue("flow_obstacle_invalid",path,"Flow obstacle geometry, interval, and derived grid mask must match exactly");
  }
