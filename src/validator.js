...
export async function validateAuthoredPackage(packageValue) {
  ...
  for(let index=0;index<flowTraces.length;index+=1)if(isPlainRecord(flowTraces[index])&&flowTraces[index].converterProfile!==undefined)issue("converter_profile_flow_trace_forbidden",`conversionTrace.flow[${index}].converterProfile`,"Flow traces must not carry Boxing converter profile provenance");
