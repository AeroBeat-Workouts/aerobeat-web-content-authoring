...
export async function validateAuthoredPackage(packageValue) {
  ...
  for (let index = 0; index < charts.length; index += 1) {
    ...
    if (chart.mode === "flow") { flowCount += 1; if(chart.schemaId!=="aerobeat.chart.flow.v3"||chart.schemaVersion!==3||chart.rulesetId!=="flow_grid_v2")issue("flow_chart_schema_invalid",path,"Flow chart must use normalized obstacle schema/ruleset v3"); const obstacleCount=chart.beats.filter((beat)=>isPlainRecord(beat)&&beat.type==="obstacle").length;if(obstacleCount>maximumObstaclesPerChart)issue("flow_obstacle_limit_exceeded",`${path}.beats`,`Flow chart exceeds the obstacle limit`);for (let beatIndex = 0; beatIndex < chart.beats.length; beatIndex += 1) validateFlowBeat(chart.beats[beatIndex], `${path}.beats[${beatIndex}]`, issue); continue; }
