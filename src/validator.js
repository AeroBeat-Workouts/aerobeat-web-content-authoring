...
export async function validateAuthoredPackage(packageValue) {
  ...
  if(isPlainRecord(packageValue.source)){
    ...
    if(!infoFormatValid||!infoVersionValid||!validHash(packageValue.source.sourceInfoHash)||!beatmapFormatValid||!beatmapVersionValid)issue("source_format_provenance_invalid","source","Package source must retain exact independent Info and beatmap format provenance");
