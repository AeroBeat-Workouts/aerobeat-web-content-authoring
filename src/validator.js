...
import { authoredPackageSchemaId, authoredPackageSchemaVersion, authoredPackageVersion, flowChartSchemaId, flowChartSchemaVersion, flowContentIdentity, flowGridRulesetId, hasExactFlowRulesetVariants } from "./flow-contract.js";
...
export async function validateAuthoredPackage(packageValue) {
  ...
  for (let index = 0; index < charts.length; index += 1) {
    ...
    if (chart.mode === "flow") { flowCount += 1; if(chart.schemaId!==flowChartSchemaId||chart.schemaVersion!==flowChartSchemaVersion||chart.rulesetId!==flowGridRulesetId||!hasExactFlowRulesetVariants(chart.rulesetVariants))issue("flow_chart_schema_invalid",path,"Flow chart must explicitly bind flow_grid_v2 and flow_colliders_v1 in canonical order");if(!canonicalEqual(chart.notePalette,expectedPaletteReference))issue("flow_note_palette_mismatch",`${path}.notePalette`,"Flow chart must reference the package note palette exactly");const expectedFlowContentHash=await prefixedSha256(canonicalJson(flowContentIdentity(chart.beats,expectedPaletteReference)));if(chart.contentHash!==expectedFlowContentHash)issue("flow_content_hash_mismatch",`${path}.contentHash`,"Flow content hash must bind beats, exact ruleset variants and palette reference");const flowTrace=flowTraces[0];if(isPlainRecord(flowTrace)&&flowTrace.contentHash!==chart.contentHash)issue("flow_trace_content_hash_mismatch","conversionTrace.flow[0].contentHash","Flow trace must bind exact Flow content hash"); const obstacleCount=chart.beats.filter((beat)=>isPlainRecord(beat)&&beat.type==="obstacle").length;if(obstacleCount>maximumObstaclesPerChart)issue("flow_obstacle_limit_exceeded",`${path}.beats`,`Flow chart exceeds the obstacle limit`);for (let beatIndex = 0; beatIndex < chart.beats.length; beatIndex += 1) validateFlowBeat(chart.beats[beatIndex], `${path}.beats[${beatIndex}]`, issue); continue; }
