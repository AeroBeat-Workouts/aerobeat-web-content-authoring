...
function syntheticSource() {
  return sourceForDocument("v3", { version: "3.3.0", colorNotes: [{ b: 1, x: 0, y: 2, c: 0, d: 1 }, { b: 2, x: 1, y: 1, c: 1, d: 0 }, { b: 3, x: 2, y: 0, c: 0, d: 2 }], bombNotes: [], obstacles: [{ b: 4, d: 1, x: 0, y: 0, w: 1, h: 3 }], sliders: [], burstSliders: [] });
}

/** @param {"v2" | "v3" | "v4"} format */
function malformedSource(format) {
  const field = format === "v2" ? "_obstacles" : "obstacles";
  return sourceForDocument(format, { [field]: { malformed: true }, ...(format === "v4" ? { obstaclesData: [] } : {}) });
}

/** @param {"v2"|"v3"|"v4"} beatMapFormat @param {Record<string, unknown>} map */
function sourceForDocument(beatMapFormat, map) {
  const path = "Hard.dat";
  ...
  return Object.freeze({
    manifest: Object.freeze({ schemaId: "aerobeat.beatsaver-source-manifest.v2",infoFormatMajor:beatMapFormat==="v4"?4:2,infoFormat:beatMapFormat==="v4"?"v4":"v2",infoVersion:beatMapFormat==="v4"?"4.0.0":"2.1.0", infoPath: "Info.dat",hashInputPaths:Object.freeze([path]), songName: "Browser Synthetic",songSubName:"", songAuthorName: "AeroBeat", levelAuthorName: "AeroBeat", audioPath: "song.ogg",coverPath:"", bpm: 120,previewStartSeconds:0,previewDurationSeconds:0, difficulties: Object.freeze([{ characteristic: "Standard", difficulty: "Hard",difficultyRank:5, path,beatMapFormatMajor:Number(beatMapFormat.slice(1)),beatMapFormat,beatMapVersion:beatMapFormat==="v2"?"2.6.0":beatMapFormat==="v3"?"3.3.0":"4.0.0",notePalette:null,noteJumpMovementSpeed:10,noteJumpStartBeatOffset:0 }]), entries: Object.freeze([]),archiveBytes:0,expandedBytes:0 }),
