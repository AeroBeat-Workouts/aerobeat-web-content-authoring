...
Current output is package schema `aerobeat.song-package.v4`, schema version `4`, package version `4.0.0`, with one Flow `aerobeat.chart.flow.v4` schema version `4`. Package `notePalette` is either a hash-authenticated authored song pair with private source provenance or `null`. Flow carries only a package palette reference/hash and a content hash over its ruleset, beats, and palette reference. Palette identity is bound into package integrity, Flow trace, top conversion trace, and semantic parity, but remains absent from every beat, every Boxing chart/trace identity, and scoring semantics.
...
- memory, fake IndexedDB, and real Chromium DB7 preservation/list/export/delete/reimport/shared-asset paths, including exact `note_palette_reimport_required` behavior for v3;
