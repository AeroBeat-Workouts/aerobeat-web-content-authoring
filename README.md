...
The root module also exports the immutable consumer contract constants `authoredPackageSchemaId`, `authoredPackageSchemaVersion`, `authoredPackageVersion`, `flowChartSchemaId`, `flowChartSchemaVersion`, `flowCollidersRulesetId` (sole Flow ruleset), `flowGridRulesetId` (retired historical ID, exported only to recognize pre-rename stored bytes), and `flowRulesetVariants`. Consumers must compare exact values rather than infer support from package age, chart mode, beat content, or a missing field.
