...
IndexedDB database `aerobeat-web-content-authoring` is schema version 5 with `packages`, `assets`, `collections`, and `meta` stores. Opening a version 1–4 database atomically preserves every package byte/hash, collection, inline asset, shared asset, and optional local-ZIP source-cache byte while labeling legacy rows `bounded_mask_v1`. Stale entries remain listable with unchanged exact public summary keys, exportable for recovery, and deletable, but package load and asset/play reads fail with bounded storage code `flow_obstacle_reimport_required`; they cannot masquerade as source-geometry output. Every new memory or IndexedDB write is marked `source_geometry_v1` plus corrected top-left orientation internally. Reimport through `put` or `putCollection` replaces the same stable package/collection keys with current rows, after which normal loading succeeds, and scan-based garbage collection retains shared assets while any surviving package references them. The `aerobeat.authored-packages.v2` handle namespace remains unchanged, so assembly may retain stale entries for management but must surface the reimport requirement rather than silently selecting them. Both adapters provide atomic `putCollection`, `listCollections`, `getCollection`, and `deleteCollection` operations; quota/cancellation fail before commit, and optional source-cache entries remain package-local and disabled unless requested.
...
- real uncommitted BeatSaver fixtures `4858` Standard Expert and `3D44B` Standard Hard atomically rejected for malformed obstacle records, plus the committed exact raw `3c9d` Standard Hard source/hash oracle;
