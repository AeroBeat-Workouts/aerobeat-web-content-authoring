...
It adapts one explicit Standard difficulty, or prepares every exact Standard difficulty in canonical Easy → Normal → Hard → Expert → ExpertPlus order, using only required entry copies and one shared audio read/hash. Worker conversion remains one difficulty per request and emits one Flow chart with shared immutable beat truth for the exact ruleset variants `flow_grid_v2` and `flow_colliders_v1`, plus the four frozen Boxing prototype combinations:
...
The root module also exports the immutable consumer contract constants `authoredPackageSchemaId`, `authoredPackageSchemaVersion`, `authoredPackageVersion`, `flowChartSchemaId`, `flowChartSchemaVersion`, `flowGridRulesetId`, `flowCollidersRulesetId`, and `flowRulesetVariants`. Consumers must compare exact values rather than infer support from package age, chart mode, beat content, or a missing field.
...
Current output is the reimport-required successor package schema `aerobeat.song-package.v6`, schema version `6`, package version `6.0.0`, with immutable hash-bound `source.spawnTiming`. The exact selected-difficulty Info.dat BPM/NJS/offset are carried through the strict source manifest and Worker request, then authoring recomputes HJD/reaction time/diagnostic jump distance with pinned `beatsaber_core_hjd_v1` constants and operation order; every trace and package hash binds the same exact record. Versions 1–4 require stable `spawn_timing_reimport_required` rather than inferred defaults. Version 5 requires exact `flow_colliders_reimport_required`: neither persistence nor a new consumer may derive, append, or silently promote the missing identity.

The package retains one Flow `aerobeat.chart.flow.v5` schema version `5`. Its stable default `rulesetId` remains `flow_grid_v2`, and its exact canonical `rulesetVariants` is `["flow_grid_v2","flow_colliders_v1"]`. This is intentionally one array of identities over one authored `beats` array—not two charts or duplicated mutable beat truth. The same exact pair appears in the Flow trace. Flow `contentHash` is SHA-256 of canonical `{beats,rulesetId,rulesetVariants,notePalette}`; therefore the Flow hash, trace hash binding, package hash, Worker result hash, export metadata hash, and semantic parity hash all bind `flow_colliders_v1` explicitly. Package `notePalette` is either a hash-authenticated authored song pair with private source provenance or `null`. Flow carries only a package palette reference/hash. Palette identity remains absent from every beat, every Boxing chart/trace identity, and scoring semantics. Downstream runtime may privately apply the one package-effective pair to left/right Boxing punch rendering; authoring does not embed palette fields in Boxing charts, traces, or events.
...
IndexedDB database `aerobeat-web-content-authoring` is schema version 8 with `packages`, `assets`, `collections`, and `meta` stores. Opening a version 1–7 database atomically preserves every package byte/hash, collection, inline asset, shared asset, and optional local-ZIP source-cache byte. Existing orientation, obstacle, spawn-timing, and palette failures retain their historical precedence. An otherwise valid v5 row remains listable/exportable/deletable but normal package load and asset/play reads fail with exact `flow_colliders_reimport_required`; DB8 records only a `preserved-reimport-required` invalidation marker and never rewrites v5 bytes, hashes, Flow charts, or traces. The exact raw-0.0.39 DB5 `flowObstacleContract` package/collection field and the dual-key DB6 rows produced by the faulty DB5→DB6 migration remain validated and translated to the sole internal `obstacleContract` field; unknown shapes abort the complete upgrade with a bounded storage error. Same-source reimport creates a current v6 package without rewriting historical bytes/hashes. Every new memory or IndexedDB write is marked `normalized_obstacle_v2` plus corrected top-left orientation internally. Reimport through `put` or `putCollection` replaces the same stable package/collection keys with current rows, after which normal loading succeeds, and scan-based garbage collection retains shared assets while any surviving package references them. The `aerobeat.authored-packages.v2` handle namespace remains unchanged, so assembly may retain stale entries for management but must surface the reimport requirement rather than silently selecting them. Both adapters provide atomic `putCollection`, `listCollections`, `getCollection`, and `deleteCollection` operations; quota/cancellation fail before commit, and optional source-cache entries remain package-local and disabled unless requested.
...
- memory, fake IndexedDB, and real Chromium DB8 preservation/list/export/delete/reimport/shared-asset paths, including exact `spawn_timing_reimport_required` behavior for v1–v4 and `flow_colliders_reimport_required` for v5;
- adversarial removal, partial declaration, reordering, duplication, and attacker-rehash of the Flow ruleset variants, all rejected without persistence;
- deterministic package export whose package hash binds the successor ruleset identity;
- current sibling-runtime v6 integration plus a hash-pinned, authoring-owned historical-v5 acceptance oracle, so old-consumer fail-closed evidence cannot drift when sibling runtime support advances;
